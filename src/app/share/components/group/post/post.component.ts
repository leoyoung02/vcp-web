...
export class GroupPostComponent {
  ...
  async ngOnInit() {
    ...
    if(this.post) {
      this.content = this.post?.comment;
      if(this.content) {
  ...
  isCurrentUserLiked(post) {
    let result = false
    let liked = post.Company_Group_Comment_Reactions?.filter(p => {
  ...
  handleAddComment() {
    ...
    if(!this.newComment || this.isNewCommentPlaceholder()) {
      ...
      this._clubsService.addGroupCommentReply(this.post.id, param).subscribe(
        (data) => {
          this.open(
