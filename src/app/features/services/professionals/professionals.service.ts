...
export class ProfessionalsService {
  ...
  getProfessionalsHomeData(id, userId): Observable<any> {
    return this._http.get(`${PROFESSIONALS_HOME_DATA_URL}/${id}/${userId}`,
      { headers: this.headers }
    ).pipe(map(res => res))
  }
  ...
  getWalletData(id, userId): Observable<any> {
    return this._http.get(`${WALLET_DATA_URL}/${id}/${userId}`,
      { headers: this.headers }
    ).pipe(map(res => res))
  }

  editWalletSettings(params): Observable<any> {
    return this._http.post(
      EDIT_WALLET_SETTINGS_URL,
      params
    ).pipe(map(res => res));
  }
  ...
  configureStripeConnect(payload): Observable<any> {
    return this._http.post(`${SETUP_STRIPE_CONNECT_URL}`,
      payload,
      {headers: this.headers}
    ).pipe(map(res => res))
  }

  updatePlatformPercentage(params): Observable<any> {
    return this._http.post(EDIT_PLATFORM_PERCENTAGE_URL,
        params,
        { headers: this.headers }
    ).pipe(map(res => res));
  }
