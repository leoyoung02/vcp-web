...
export class JobOffersAdminListComponent {
    ...
    searchKeyword: any;
    dataSource: any;
    displayedColumns = ["title_display", "status_display", "type_display", "date_display", "candidates", "action"];
    pageSize: number = 25;
    pageIndex: number = 0;
        ...
        if(offers?.length > 0) {
            data = offers?.map(offer => {
                ...
                return {
                    ...
                    status_display: offer.status == 1 ? this._translateService.instant('company-settings.active') : this._translateService.instant('company-settings.inactive'),
    ...
    loadJobOffers(data) {
        ...
        if(this.searchKeyword && this.jobOffersData) {
            this.jobOffersData = this.jobOffersData.filter(offer => {
                return (offer.title && offer.title.toLowerCase().indexOf(this.searchKeyword.toLowerCase()) >= 0) ||
                    (offer.title_en && offer.title_en.toLowerCase().indexOf(this.searchKeyword.toLowerCase()) >= 0) ||
                    (offer.title_fr && offer.title_fr.toLowerCase().indexOf(this.searchKeyword.toLowerCase()) >= 0) ||
                    (offer.location && offer.location.toLowerCase().indexOf(this.searchKeyword.toLowerCase()) >= 0) ||
                    (offer.company && offer.company.toLowerCase().indexOf(this.searchKeyword.toLowerCase()) >= 0)
            })
        }
