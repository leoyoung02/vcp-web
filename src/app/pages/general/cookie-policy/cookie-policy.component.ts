...
export class CookiePolicyComponent implements OnInit, OnDestroy {
  ...
  handleEditorInit(e) {
    ...
    setTimeout(() => {
        if (this.editor && this.iframeCookiePolicy && this.policy?.changingThisBreaksApplicationSecurity) {
            this.editor.nativeElement.style.display = 'block'

            e.editor.setContent(this.policy?.changingThisBreaksApplicationSecurity)
            this.iframeCookiePolicy.nativeElement.style.height = `${e.editor.container.clientHeight + 200}px`

            this.editor.nativeElement.style.display = 'none'

            this.iframeCookiePolicy.nativeElement.src =
                ...
                '</html>';
            this.iframeCookiePolicy.nativeElement.style.display = 'block'
        }
    }, 500)
  }
