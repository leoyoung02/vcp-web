...
export class HomeComponent implements OnInit, OnDestroy {
    ...
    async getMenus() {
        this.menus = this._localService.getLocalStorage(environment.lsmenus) ? JSON.parse(this._localService.getLocalStorage(environment.lsmenus)) : []
    ...
    getPredefinedTemplate(id) {
        this._companyService.getHomeTemplate(id, this.companyId)
        .subscribe(
            response => {
                if(this.hasProfileHomeContent && response.home_template_mapping && response.home_template_mapping?.length > 1){
                    let preDefinedTemplate = response.home_template_mapping
                    this.predefinedTemplate = (response.home_template_mapping)?.filter(tmp => {
                        if(tmp.custom_member_type_id == this.customMemberTypeId){
                            if(tmp.has_title && !tmp.video_title){
                                tmp.video_title = preDefinedTemplate?.length > 0 ? preDefinedTemplate[0].video_title : ''
                            }
                            if(!tmp.video_description){
                                tmp.video_description = preDefinedTemplate?.length > 0 ? preDefinedTemplate[0].video_description : ''
                            }
                            if(tmp.has_video && !tmp.video_file){
                                tmp.video_file = preDefinedTemplate?.length > 0 ? preDefinedTemplate[0].video_file : ''
                            }
                        }
    ...
    getTitles() {
        if(this.clubsFeature) {
            this.groupsTitle = this.language == 'en' ? (this.clubsFeature.name_en || this.clubsFeature.feature_name || this.clubsFeature.name_es || this.clubsFeature.feature_name_ES) : (this.language == 'fr' ? (this.clubsFeature.name_fr || this.clubsFeature.feature_name_FR || this.clubsFeature.name_es || this.clubsFeature.feature_name_ES) : 
                    (this.language == 'eu' ? (this.clubsFeature.name_eu || this.clubsFeature.feature_name_EU || this.clubsFeature.name_es || this.clubsFeature.feature_name_ES) : (this.language == 'ca' ? (this.clubsFeature.name_ca || this.clubsFeature.feature_name_CA || this.clubsFeature.name_es || this.clubsFeature.feature_name_ES) : 
                    (this.language == 'de' ? (this.clubsFeature.name_de || this.clubsFeature.feature_name_DE || this.clubsFeature.name_es || this.clubsFeature.feature_name_ES) : (this.clubsFeature.name_es || this.clubsFeature.feature_name_ES))
                ))
            )
        }
    }
                        ...
                        this.otherSettings.forEach(m => {
                            if(m.title_es == 'General') {
                                if(m.content) {
                                    ...
                                    if(menuOrderSettings && menuOrderSettings[0]) {
                                        this.hasMenuOrdering = menuOrderSettings[0].active == 1 ? true : false
                                    }
                                }
                            ...
                            if(m.title_es == 'Registro / Servicios') {
                                if(m.content) {
                                    ...
                                    if(shareRegistrationLinkSettings && shareRegistrationLinkSettings[0]) {
                                        this.canShareRegistrationLink = shareRegistrationLinkSettings[0].active == 1 ? true : false
    ...
    updateDisabledFeatures(menus) {
        let activity = menus && menus.filter(menu => { return menu.id == 1 && menu.path == 'plans' })
        if(!activity || (activity && activity.length == 0)) { this.disableActivities = true }

        let club = menus && menus.filter(menu => { return menu.id == 5 })
        if(!club || (club && club.length == 0)) { this.disableClubs = true }

        let joboffers = menus && menus.filter(menu => { return menu.id == 18 })
        if(!joboffers || (joboffers && joboffers.length == 0)) { this.disableJobOffers = true }

       let courses = menus && menus.filter(menu => { return menu.id == 11 })
        if(!courses || (courses && courses.length == 0)) { this.disableCourses = true }

        let members = menus && menus.filter(menu => { return menu.id == 15 })
        if(!members || (members && members.length == 0)) { this.disableMembers = true }
    }

    getMobileLimitSettings() {
        this._companyService.getMobileLimitSettingsAll(this.companyId)
        .pipe(takeUntil(this._destroy$))
        .subscribe(data =>{
            let settings = data.mobile_limit_settings
            
            let activity_limit = settings &&settings.filter(s => {
            ...
            let cityagenda_limit = settings &&settings.filter(s => {
            ...
            let club_limit = settings &&settings.filter(s => {
            ...
            let joboffers_limit = settings &&settings.filter(s => {
            ...
            let discounts_limit = settings &&settings.filter(s => {
            ...
            let courses_limit = settings &&settings.filter(s => {
            ...
            let startups_limit = settings &&settings.filter(s => {
    ...
    getLandingTemplateBySlug(slug) {
        this._companyService.getLandingTemplateBySlug(slug)
        .pipe(takeUntil(this._destroy$))
        .subscribe(data =>{
        ...
        if(this.companyTemplate) {
            ...
            setTimeout(() => {
                ...
                if(table) {
                    ...
                    sheet.innerHTML = ".guest-home.html-container strong, .guest-home.html-container p, .guest-home.html-container h1, .guest-home.html-container h2, .guest-home.html-container h3, .guest-home.html-container h4, .guest-home.html-container h5, .guest-home.html-container span {font-family: " + table.style.fontFamily + ";} ";
    ...
    hasAccess(path) {
        ...
        if(this.menus?.length > 0) {
            let feature = this.menus && this.menus.filter(m => {
                return m.path == path
            })
            if(feature && feature.length > 0) { access = true }
        }
